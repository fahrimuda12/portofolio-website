...
class TimezoneHelper
{
    ...
    {
        $getTimezone = json_decode(file_get_contents(self::baseUrl($ip)));
        $timezone = isset($getTimezone->timezone) ? $getTimezone->timezone : env('APP_TIMEZONE');
        return $timezone;
    }

    public static function getSelisihTimezone($timezone)
    {
        $selisih = array_search($timezone, self::allTimezoneList());
        if (!$selisih) {
            return self::getSelisihTimezone('UTC');
        }

        $selisih = clean_explode($selisih, ' ');
        $selisih = str_replace(['(UTC',':00)'], '', $selisih[0]);
        return $selisih;
    }

    private static function baseUrl($ip)
    {
        return self::BASE_URL.$ip.'/json';
    }

    private static function allTimezoneList()
    {
        return [
